...
void View::drawBall(std::vector<Ball> balls) {
    for (auto& ball: balls){
        al_draw_filled_circle(ball.getCenter().x_, ball.getCenter().y_, ball.getRadius(), al_map_rgb(151, 154, 218));
    }
};
...
void View::drawBricks(std::vector<std::vector<Brick>> bricks) {
    for (auto& bricks_row: bricks) {
        for (auto& brick: bricks_row) {
            ...
            al_draw_filled_rectangle(x1, y1, x2, y2, al_map_rgb(123, 132, 25));
        }
    }
};
